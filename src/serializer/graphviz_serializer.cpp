...
GraphVizSerializer::GraphVizSerializer(const IGraph& graph) : graph_(graph) {
}

std::pair<size_t, size_t> GraphVizSerializer::normalizeEdge(size_t u,
                                                            size_t v) const {
...
std::string GraphVizSerializer::serializeFull(
    const std::string& title, const std::map<size_t, VertexStyle>& vertexStyles,
...
void GraphVizSerializer::serializeFull(
    ...
    for (size_t u = 0; u < graph_.getV(); ++u) {
        ...
        for (size_t v : neighbors) {
            ...
            bool isCycleEdge =
                (cycleEdgeSet.find({u, v}) != cycleEdgeSet.end());
